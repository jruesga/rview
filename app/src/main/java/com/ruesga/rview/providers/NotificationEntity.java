...
public class NotificationEntity implements BaseColumns, Parcelable {
    ...
    private static final int NOTIFICATION_IDX = 4;
    ...
    public static List<NotificationEntity> getAllNotifications(Context context) {
        List<NotificationEntity> stats = new ArrayList<>();
        ContentResolver cr = context.getContentResolver();
        String sort = ACCOUNT_ID + " ASC, " + GROUP_ID + " ASC, " + WHEN + " ASC";
        Cursor c  = cr.query(CONTENT_URI, ALL_FIELDS_PROJECTION, null, null, sort);
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    do {
    ...
    public static List<NotificationEntity> getAllGroupNotifications(Context context, long groupId) {
        List<NotificationEntity> stats = new ArrayList<>();
        ContentResolver cr = context.getContentResolver();
    ...
    public static List<NotificationEntity> getAllAccountNotifications(
            Context context, String accountId) {
        List<NotificationEntity> stats = new ArrayList<>();
        ContentResolver cr = context.getContentResolver();
    ...
    public static void deleteGroupNotifications(Context context, int groupId) {
        ContentResolver cr = context.getContentResolver();
        String where = GROUP_ID + " = ?";
